...
def get_schema(table_name: str) -> list[dict]:
    """Return column definitions for a table in MYSQL_DATABASE.

    Each row contains Field, Type, Null, Key, Default, and Extra.
    The name is treated as a single identifier, not a database.table path.
    """
    if not isinstance(table_name, str) or not table_name.strip():
        raise ValueError("table_name must be a non-empty string")
    # Identifiers cannot use SQL value placeholders; escape backticks instead.
    quoted_name = table_name.replace("`", "``")
    return execute_query(f"DESCRIBE `{quoted_name}`")
